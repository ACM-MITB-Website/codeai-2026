    ...
    return (
        <div className="hero-image-section relative group w-full h-full overflow-hidden bg-[#1a2332]">
            ...
            {slides.map((slideGroup, index) => (
                <div
                    key={`fg-${index}`}
                    className={`absolute inset-0 flex items-center justify-center p-4 md:p-12 transition-all duration-700 ease-in-out transform ${index === currentIndex
                ...
                >
                    <div className={`flex flex-col md:flex-row gap-4 h-full w-full items-center justify-center ${slideGroup.length > 1 ? 'max-w-7xl' : 'max-w-5xl'}`}>
                        {slideGroup.map((img, imgIdx) => (
                            <div
                                key={imgIdx}
                                className={`relative overflow-hidden rounded-xl shadow-2xl transition-transform hover:scale-[1.02] duration-300 ${slideGroup.length === 1 ? 'w-full h-full' :
                                        slideGroup.length === 2 ? 'w-full md:w-1/2 h-4/5' :
            ...
            <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-2 z-20">
                {slides.map((_, index) => (
                    <button
                        ...
                        className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-[#ffd700] w-8' : 'bg-white/50 hover:bg-white'
                            }`}
